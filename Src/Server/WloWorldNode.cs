...
namespace Server
{
    public class WloWorldNode
    {
        ...
        List<WloClient> m_localPlayerlist, m_externalPlayerlist;

        ConcurrentStack<WloClient> ConnectedQueue, DisconnectedQueue;

        //Queue<Player> ConnectedQueue, DisconnectedQueue;
        ...
        public WloWorldNode()
        {
            ...
            m_localPlayerlist = new List<WloClient>();
            m_externalPlayerlist = new List<WloClient>();
            ConnectedQueue = new ConcurrentStack<WloClient>();
            DisconnectedQueue = new ConcurrentStack<WloClient>();
        }

        #region Events
        void c_onNewClient(object sender, Phoenix.Core.Networking.Client3 e)
        {
            ConnectedQueue.Push(new Player(e));
        }
        ...
        //public event Action<Player> onPlayerDced;
        #endregion
        ...
        public void Open()
        {
            ...
            var thrd = new Thread(new ThreadStart(MainMapLoop));
            thrd.IsBackground = true;
            thrd.Init();
            
            DebugSystem.Write("Starting TCP Listeners");
            m_listenerlist.Add(new Listeners.TcpListener(6414));
            m_listenerlist.Add(new Listeners.TcpListener(25563));

            m_listenerlist.ForEach(new Action<Listeners.TcpListener>(c =>
            {
                c.onNewClient += c_onNewClient;
                c.Initialize();
            }));
        ...
        {
            playerList.AsParallel().ForAll(c => c.Disconnect());
            GameEvents.Stop();
            Killnow = true;
            DebugSystem.Write("Closing Listeners");
            m_listenerlist.ForEach(c => c.Kill());
        }
            ...
            do
            {
                ...
                if (ConnectedQueue.TryPopRange(pitems, 0, 50) > 0)
                {
                    try
                    {
                        ...
                        playerList.AddRange(pitems.Where(c => c != null).ToArray());
        ...
        /// <summary>
        /// Broadcasts a Packet to certain people who are in a Map
        /// </summary>
        /// <param name="pkt"></param>
        /// <param name="To">"Multiple target IDs as string to send to specific people"</param>
        public void Broadcast(Packet pkt, string parameter, params object[] To)
        {
        ...
        public bool DoAction(uint src,Action<Player> work)
        {
            try
            {
                ...
                {
                    work.Invoke(playerList.Single(c => c.CharID == src));
                    return true;
                }
            }
        ...
        public async void onTelePort(TeleportType teletype, byte portalID, WarpData map, Player target)
        {
            using (var releaser = await listlock.LockAsync())
            {
                ...
                {
                    //Create Map
                    GameMap tmp = cGlobal.gMapManager.GetMap(map.DstMap);
        ...
        public async void CommenceLogin(Player src)
        {
            ...
            src.SendPacket(Packet.ConvertfromFormat<Packet>("bbbw", 24, 5, 183, 0));
            src.SendPacket(Packet.ConvertfromFormat<Packet>("bbbw", 24, 5, 53, 0));
            src.SendPacket(Packet.ConvertfromFormat<Packet>("bbbw", 24, 5, 52, 0));
            src.SendPacket(Packet.ConvertfromFormat<Packet>("bbbw", 24, 5, 54, 0));
            src.SendPacket(Packet.ConvertfromFormat<Packet>("bbbswb", 70, 1, 23, "Something", 194, 0));
            src.SendPacket(Packet.ConvertfromFormat<Packet>("bbb", 20, 33, 0));
            //-----Player Stats values--------
            src.Send8_1(false);
            Thread.Sleep(5);
            src.SendPacket(Packet.ConvertfromFormat<Packet>("bbb", 14, 13, 3));
            //-----Im Mall List
            // //g.ac75.Send_1(g.gImMall_Manager.Get_75IM);
            src.SendPacket(Packet.ConvertfromFormat<Packet>("bbw", 75, 8, 0));
            ...
            //------------Player Data---------------------
            src.Send_5_3();
            src.SendPacket(Packet.FromArray(src.Inv.GetAC23_5()));
            src.SendPacket(Packet.FromArray(src._23_11Data));
            //-------------------------Quest----------------------
            ...
            // //    g.SetSize();
            // //    Send(g);
            src.SendPacket(Packet.ConvertfromFormat<Packet>("bbd", 26, 4, src.Gold));
            src.SendPacket(Packet.FromArray(src.Settings.ToArray()));
            ...
            src.SendPacket(Packet.ConvertfromFormat<Packet>("bbb", 5, 15, 0));
            src.SendPacket(Packet.ConvertfromFormat<Packet>("bbw", 62, 53, 2));
            src.SendPacket(Packet.ConvertfromFormat<Packet>("bbb", 5, 21, src.Slot));
            src.SendPacket(Packet.ConvertfromFormat<Packet>("bbdw", 5, 11, 15085, 5000));
            // //g.ac5.Send_11(15085, 0);//244, 68, 8, 0, 5, 11, 237, 58, 0, 0, 0, 0, 
            //---------------------------------
            //g.ac62.Send_4(g.packet.cCharacter.cCharacterID); //tent items
            //--------------------------------------
            src.SendPacket(Packet.ConvertfromFormat<Packet>("bbb", 5, 14, 2));
            src.SendPacket(Packet.ConvertfromFormat<Packet>("bbb", 5, 16, 0));
            src.SendPacket(Packet.ConvertfromFormat<Packet>("bbbl", 23, 140, 3, DateTime.Now.ToOADate()));
            src.SendPacket(Packet.ConvertfromFormat<Packet>("bbbl", 25, 44, 2, DateTime.Now.ToOADate()));
            // //g.ac23.Send_106(1, 1);
            src.SendPacket(Packet.ConvertfromFormat<Packet>("bbb", 23, 160, 3));
            src.SendPacket(Packet.ConvertfromFormat<Packet>("bbb", 75, 7, 1));
            src.SendPacket(Packet.ConvertfromFormat<Packet>("bbbs", 23, 57, 0, "Welcome to the  WLO 4 EVER Community Server :! Enjoy !!"));
            src.SendPacket(Packet.ConvertfromFormat<Packet>("bbb", 69, 1, 71));
            src.SendPacket(Packet.ConvertfromFormat<Packet>("bbb", 20, 60, 1));
            src.SendPacket(Packet.FromArray(new byte[] { 66, 1, 001, 012, 043, 000, 000, 000, 000, 000, 000, 000, 000 }));

            for (byte a = 1; a < 11; a++)
                src.SendPacket(Packet.ConvertfromFormat<Packet>("bbbw", 5, 13, a, 0));
            for (byte a = 1; a < 11; a++)
                src.SendPacket(Packet.ConvertfromFormat<Packet>("bbbw", 5, 24, a, 0));

            src.SendPacket(Packet.ConvertfromFormat<Packet>("bbbw", 23, 162, 2, 0));
            src.SendPacket(Packet.ConvertfromFormat<Packet>("bbd", 26, 10, 0));
            src.SendPacket(Packet.ConvertfromFormat<Packet>("bbw", 23, 204, 1));
            src.SendPacket(Packet.ConvertfromFormat<Packet>("bbbbd", 23, 208, 2, 3, 0));
            src.SendPacket(Packet.ConvertfromFormat<Packet>("bbbbd", 23, 208, 2, 4, 0));
            src.SendPacket(Packet.ConvertfromFormat<Packet>("bb", 1, 11));
            src.SendPacket(Packet.ConvertfromFormat<Packet>("bbbbbb", 15, 19, 4, 6, 9, 94));
            src.SendPacket(Packet.FromArray(new byte[] { 54, 89, 2, 2, 90, 2, 1, 91, 2, 1, 189, 2, 2, 190, 2, 1, 191, 2, 1 }));
            src.SendPacket(Packet.ConvertfromFormat<Packet>("bbdddd", 35, 4, 0, 0, 0, 0));//first 0 is im
            src.SendPacket(Packet.ConvertfromFormat<Packet>("bbbbbb", 90, 1, 0, 2, 2, 3));
